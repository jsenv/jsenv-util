...
try {
  ...
  const expected = new TypeError("fileUrl must be a string or an url, received undefined")
...
try {
  ...
  const expected = new Error(`fileUrl must starts with file://, received http://example.com`)
  assert({ actual, expected })
}
