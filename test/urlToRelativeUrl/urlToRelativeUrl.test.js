...
{
  const actual = urlToRelativeUrl("http://user:pass@example.org/foo/bar", "http://example.org/foo/")
  const expected = "//user:pass@example.org/foo/bar"
  assert({ actual, expected })
}
...
{
  const actual = urlToRelativeUrl("http://example.org/foo/bar", "http://user:pass@example.org/foo/")
